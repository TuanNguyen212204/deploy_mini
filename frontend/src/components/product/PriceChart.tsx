...
// Màu fallback nếu platform không có trong danh sách
...
export default function PriceChart({
  ...
  const chartData = useMemo(() => {
    ...
    // Tìm min/max chung của tất cả platform để scale đồng nhất
    ...
    const platformChartData = platforms.map((platform, pIdx) => {
      const points = platform.prices.map((pt, index) => {
        ...
        const y =
            height -
            paddingY -
            ((pt.price - globalMin) / range) * (height - paddingY * 2);
  ...
  const activePoint =
      hoverInfo !== null
          ? platformChartData[hoverInfo.platformIndex]?.points[hoverInfo.pointIndex]
          : null;
  const activePlatform =
      hoverInfo !== null ? platformChartData[hoverInfo.platformIndex] : null;
  ...
  const handlePointerMove = (
    ...
    const points = platformChartData[platformIndex]?.points ?? [];
    ...
    setHoverInfo({ platformIndex, pointIndex: closestIndex });
  };
  ...
  return (
      <div className="rounded-[30px] border border-stone-200/80 bg-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
        <div className="max-w-2xl">
          <h3
              className="text-3xl tracking-normal text-stone-900"
        ...
        {/* Legend — hiển thị màu từng platform */}
          ...
          <div className="relative">
            ...
            >
              <defs>
                {platformChartData.map((p) => (
                    <linearGradient
                        key={`grad-${p.platformName}`}
                        id={`grad-${p.platformName}`}
                        x1="0" y1="0" x2="0" y2="1"
                    >
              ...
              {/* Vẽ area fill trước (dưới) */}
              {platformChartData.map((p) =>
                  p.area ? (
                      <polygon
                          key={`area-${p.platformName}`}
                          points={p.area}
                          fill={`url(#grad-${p.platformName})`}
                      />
                  ) : null,
              )}
              ...
              {/* Invisible overlay per platform để bắt hover */}
              {platformChartData.map((p, pIdx) => (
                  <rect
                      key={`hover-${p.platformName}`}
                      x="0"
                      y="0"
                      width={width}
                      height={height}
        ...
        {/* Thống kê từng platform */}
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {platformChartData.map((p) => (
              <div
                  key={p.platformName}
                  className="rounded-[16px] border border-stone-100 bg-[#FAFAF9] px-4 py-3"
              >
                <p
                ...
                <p className="mt-2 text-lg font-semibold text-stone-900">
                  {formatPrice(p.latestPrice)}
                </p>
                <p className="text-xs text-stone-400">
                  TB 30 ngày: {formatPrice(p.averagePrice)}
                </p>
              </div>
          ))}
        </div>
        ...
        <span>
          Thấp nhất:{' '}
          <span className="font-medium text-stone-900">{formatPrice(globalMin)}</span>
        </span>
          <span>
          Cao nhất:{' '}
            <span className="font-medium text-stone-900">{formatPrice(globalMax)}</span>
        </span>
        </div>
      </div>
  );
